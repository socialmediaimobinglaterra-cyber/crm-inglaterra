...
export function CatalogGallery({photos,onChange,pending,externalCount,uploadAvailable,subject='imóvel',title='Fotos'}:{photos:DraftPhoto[];onChange:(photos:DraftPhoto[])=>void;pending:boolean;externalCount:number;uploadAvailable:boolean;subject?:string;title?:string}) {
  ...
  function select(files:FileList|null) {
    ...
    if(photos.length+selected.length>1000 || selected.some(file=>!file.size || file.size>4_000_000 || !['image/jpeg','image/png','image/webp'].includes(file.type))) {
      setMessage(`Selecione JPEG, PNG ou WebP de até 4 MB por foto. Limite de 1.000 fotos por ${subject}.`);
  ...
  return <section className="border-t border-slate-200 py-7" aria-labelledby="gallery-heading" aria-busy={pending}>
    <div className="mb-5 flex flex-wrap items-center justify-between gap-4">
      <h2 id="gallery-heading" className="text-lg font-semibold">{title} ({photos.length})</h2>
    ...
    {!photos.length?<p className="py-6 text-slate-500">Nenhuma foto selecionada.</p>:<div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{photos.map((photo,index)=><figure key={photo.key} className="min-w-0 overflow-hidden rounded-lg border border-slate-200 bg-white">
      ...
      <img src={photo.preview} alt={`Foto ${index+1} do ${subject}`} className="aspect-[4/3] w-full bg-slate-100 object-contain"/>
